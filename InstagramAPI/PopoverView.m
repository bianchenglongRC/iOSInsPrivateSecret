...
//        self.borderColor = RGB(200, 199, 204);
        ...
        self.backgroundColor  = [UIColor colorWithPatternImage:[UIImage imageNamed:@"pop_up_bg"]];
...
-(CGRect)getViewFrame
{
    ...
    frame.size.width = 568/2;
    frame.size.height = 736/2;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ChangeUserCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[ChangeUserCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
...
//    cell.backgroundView.backgroundColor = RGB(245, 245, 245);
    ...
    if ([_imageArray count] == [_titleArray count]) {
        ...
        if ([imageName rangeOfString:@"png"].length == 0) {
            [cell.headerImgView sd_setImageWithURL:[NSURL URLWithString:imageName]];

        } else {
            cell.headerImgView.image = [UIImage imageNamed:[_imageArray objectAtIndex:indexPath.row]];
        }
        
    }
    cell.nameLbl.font = [UIFont systemFontOfSize:16];
    cell.nameLbl.text = [_titleArray objectAtIndex:indexPath.row];
